...
void
test_itty_vocabulary_decode_nearest (void)
{
        const char *text_content = " apple\n banana\n cherry\n";
        size_t bit_string_words[] = { 0x01, 0x06, 0x08 };
        ...
        itty_bit_string_append_word (activation, 0x07);
...
test_itty_vocabulary_decode_nearest_rejects_uneven_fold (void)
{
        ...
        itty_bit_string_t *activation = itty_bit_string_new (ITTY_BIT_STRING_MUTABILITY_READ_WRITE);
        itty_bit_string_append_word (activation, 0x01);
        itty_bit_string_append_word (activation, 0x01);
        itty_bit_string_append_word (activation, 0x01);

        char *text = NULL;
        assert (!itty_vocabulary_decode_nearest (vocabulary, activation, &text, NULL));
        assert (text == NULL);
